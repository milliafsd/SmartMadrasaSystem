# ================== حصہ 1: Imports, DB Setup, Styles, Login ==================
...
# ------------------ Directories ------------------
for folder in ["student_photos","qr_codes","reports","graphs"]:
...
# ------------------ Login System ------------------
st.markdown("<div class='main-header'><h1>🕌 جامعہ ملیہ حفظ سسٹم</h1><p>اسمارٹ تعلیمی و انتظامی پورٹل</p></div>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.button("داخل ہوں"):
            ...
            if res:
                ...
                st.error("❌ غلط معلومات! دوبارہ کوشش کریں۔")
    st.stop()

# ------------------ Sidebar Menu ------------------
if st.session_state.user_type=="admin":
    menu = ["🏠 ڈیش بورڈ","👨‍🎓 طلباء / طالبات","📝 تعلیمی اندراج","📊 رپورٹ","🔎 تلاش","🪪 QR کارڈ","💾 بیک اپ"]
else:
    menu = ["📝 تعلیمی اندراج","📊 رپورٹ","🪪 QR کارڈ","🕒 میری حاضری","📩 درخواستِ رخصت"]
...
if st.sidebar.button("🚪 لاگ آؤٹ"):
    ...
    st.experimental_rerun()
# ================== حصہ 2: Student/Teacher Management + Hifz Entry + Reports ==================
...
def generate_html_print(df, title):
    ...
    return f'<a href="data:text/html;base64,{b64}" download="{title}.html"><button style="width:100%;background-color:#008CBA;color:white;padding:10px;border:none;border-radius:5px;font-weight:bold;">🖨️ پرنٹ / ڈاؤنلوڈ: {title}</button></a>'
...
if m=="👨‍🎓 طلباء / طالبات" and st.session_state.user_type=="admin":
    st.header("👨‍🎓 طلباء / طالبات کا انتظام")
    tab1,tab2 = st.tabs(["👥 استاذہ","🎓 طلباء"])

    with tab1:
        st.subheader("نئی استانی شامل کریں")
        with st.form("add_teacher"):
            t_name = st.text_input("نام استانی")
            ...
            if st.form_submit_button("محفوظ کریں"):
                ...
                st.experimental_rerun()
        st.subheader("موجودہ استاذہ")
        df_t = pd.read_sql_query("SELECT id,name,phone FROM teachers WHERE name!='admin'",conn)
        st.dataframe(df_t,use_container_width=True)
        del_t_id = st.number_input("حذف کرنے کے لیے ID",min_value=0,step=1,key="del_t")
        if st.button("حذف کریں"): execute_delete("teachers",del_t_id)
    ...
    with tab2:
        ...
        del_s_id = st.number_input("حذف کرنے کے لیے ID",min_value=0,step=1,key="del_s")
        if st.button("حذف کریں"): execute_delete("students",del_s_id)

# ------------------ Teacher: Daily Hifz Entry ------------------
    ...
    if not students: st.warning("آپ کی کلاس میں کوئی طالبہ نہیں ہے۔")
    else:
        for s,f in students:
            with st.expander(f"👤 {s} بنت {f}"):
                att = st.radio("حاضری",["حاضر","غیر حاضر","رخصت"],horizontal=True,key=f"att_{s}")
                if att=="حاضر":
                    # سبق
                    ...
                    if not sq_nagha:
                        ...
                        for i in range(st.session_state[f"sq_c_{s}"]):
                            ...
                            a = c3.number_input("اٹکن",0,key=f"sqa_{s}_{i}")
                            e = c4.number_input("غلطی",0,key=f"sqe_{s}_{i}")
                            sq_list.append(f"{p}:{v}")
                            sq_a += a
                            sq_m += e
                        if st.button("➕ مزید سبقی",key=f"add_sq_{s}"): st.session_state[f"sq_c_{s}"]+=1; st.experimental_rerun()
                    else: sq_list=["ناغہ"]
                    ...
                    if not m_nagha:
                        ...
                        for i in range(st.session_state[f"m_c_{s}"]):
                            c1,c2,c3,c4 = st.columns([2,2,1,1])
                            p = c1.selectbox("پارہ",paras,key=f"mp_{s}_{i}")
                            v = c2.selectbox("مقدار",["مکمل","آدھا","پون","پاؤ"],key=f"mv_{s}_{i}")
                            a = c3.number_input("اٹکن",0,key=f"ma_{s}_{i}")
                            e = c4.number_input("غلطی",0,key=f"me_{s}_{i}")
                            m_list.append(f"{p}:{v}")
                            m_a+=a
                            m_m+=e
                        if st.button("➕ مزید منزل",key=f"add_m_{s}"): st.session_state[f"m_c_{s}"]+=1; st.experimental_rerun()
                    else: m_list=["ناغہ"]
                    ...
                    if st.button("محفوظ کریں",key=f"save_{s}"):
                        c.execute("INSERT INTO hifz_records (r_date,s_name,f_name,t_name,surah,sq_p,sq_a,sq_m,m_p,m_a,m_m,attendance) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                                  (str(sel_date),s,f,st.session_state.username,sabq_final," | ".join(sq_list),sq_a,sq_m," | ".join(m_list),m_a,m_m,att))
                        conn.commit()
...
# ------------------ Daily Report ------------------
    ...
    query = "SELECT r_date as 'تاریخ', s_name as 'نام', f_name as 'ولدیت', t_name as 'استاد', attendance as 'حاضری', surah as 'سبق', sq_p as 'سبقی', sq_m as 'سبقی غلطی', m_p as 'منزل', m_m as 'منزل غلطی' FROM hifz_records WHERE r_date BETWEEN ? AND ?"
    params = [str(d1),str(d2)]
    if sel_t!="تمام اساتذہ": query += " AND t_name=?"; params.append(sel_t)
    query += " ORDER BY r_date DESC"
    df = pd.read_sql_query(query,conn,params=params)
    if df.empty: st.info("اس تاریخ کے مطابق کوئی ریکارڈ نہیں ملا۔")
    else:
        st.markdown(generate_html_print(df,"یومیہ تعلیمی رپورٹ"),unsafe_allow_html=True)
        st.dataframe(df,use_container_width=True)
# ================== حصہ 3: Exams + QR + Attendance + Leave + Monthly Report + Graphs + Backup ==================

# ------------------ Exams Module ------------------
elif m=="🎓 امتحانات":
    st.header("🎓 امتحانات و نتائج")
    if st.session_state.user_type=="teacher":
        ...
        st.subheader("تمام امتحانات")
        df_exams = pd.read_sql_query("SELECT id,s_name,f_name,para_no,total,grade,end_date,status FROM exams ORDER BY start_date DESC",conn)
        st.dataframe(df_exams,use_container_width=True)
        if not df_exams.empty: st.markdown(generate_html_print(df_exams,"امتحانی ریکارڈ"),unsafe_allow_html=True)

# ------------------ QR Module ------------------
elif m=="🪪 QR کارڈ":
    st.header("🪪 طالبہ QR کارڈ")
    students = c.execute("SELECT id,name,father_name FROM students").fetchall()
    if not students: st.warning("کوئی طالبہ موجود نہیں۔")
    else:
        sel_s = st.selectbox("طالبہ منتخب کریں",[f"{s[1]} بنت {s[2]}" for s in students])
        if st.button("QR بنائیں / ڈاؤنلوڈ کریں"):
            sn,fn = sel_s.split(" بنت ")
            qr_file = f"qr_codes/{sn}_{fn}.png"
            qr = qrcode.make(f"طالبہ: {sn} بنت {fn}")
            qr.save(qr_file)
            st.image(qr_file,width=200)
            with open(qr_file,"rb") as f:
                st.download_button("📥 QR ڈاؤنلوڈ کریں",f,file_name=f"{sn}_{fn}_QR.png")

# ------------------ Attendance Module ------------------
elif m=="🕒 میری حاضری" and st.session_state.user_type=="teacher":
...
elif m=="📩 درخواستِ رخصت" and st.session_state.user_type=="teacher":
...
elif m=="🏛️ مہتممہ پینل (رخصت)" and st.session_state.user_type=="admin":
    ...
    if not reqs: st.info("اس وقت کوئی نئی درخواست نہیں۔")
    for rid,tn,lt,d,rsn,sd in reqs:
        with st.expander(f"درخواست: {tn} ({lt} - {d} دن)"):
            st.write(f"**تفصیل:** {rsn}")
            st.write(f"**تاریخ آغاز:** {sd}")
            c1,c2 = st.columns(2)
            if c1.button("✅ منظور کریں",key=f"app_{rid}"): c.execute("UPDATE leave_requests SET status='منظور' WHERE id=?",(rid,))
            if c2.button("❌ مسترد کریں",key=f"rej_{rid}"): c.execute("UPDATE leave_requests SET status='مسترد' WHERE id=?",(rid,))
    conn.commit()
...
# ------------------ Graphs ------------------
    ...
    st.header("📈 حاضری اور غلطیوں کے گراف")
    df_graph = pd.read_sql_query("SELECT s_name, SUM(sq_m) as sq_err, SUM(m_m) as m_err FROM hifz_records GROUP BY s_name",conn)
    if not df_graph.empty:
        fig,ax = plt.subplots()
        ax.bar(df_graph['s_name'],df_graph['sq_err'],label="سبقی غلطیاں")
        ax.bar(df_graph['s_name'],df_graph['m_err'],bottom=df_graph['sq_err'],label="منزل غلطیاں")
        ax.set_xticklabels(df_graph['s_name'],rotation=45,ha='right')
        ax.set_ylabel("غلطیاں")
        ax.set_title("طالبات کی غلطیوں کا مجموعہ")
        ax.legend()
        st.pyplot(fig)
    else: st.info("کوئی ڈیٹا موجود نہیں۔")

# ------------------ Backup Module ------------------
elif m=="💾 بیک اپ" and st.session_state.user_type=="admin":
    st.header("💾 سسٹم بیک اپ")
    backup_file = f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    if st.button("💾 بیک اپ بنائیں"):
        conn.close()
        shutil.copy(DB_NAME,backup_file)
        st.success(f"بیک اپ محفوظ ہو گیا: {backup_file}")
        conn = sqlite3.connect(DB_NAME, check_same_thread=False)
        c = conn.cursor()
