...
def init_db():
    ...
    # مائیگریشن: پرانے ریکارڈز میں student_id شامل کریں
...
def download_db_from_github():
    try:
        ...
        try:
            contents = repo.get_contents(DB_NAME)
            ...
            if e.status == 404:
                ...
                st.warning(f"GitHub سے ڈاؤن لوڈ میں خرابی: {e}")
                return False
    except Exception as e:
        st.warning(f"GitHub کنکشن میں مسئلہ: {e}")
        return False

def upload_db_to_github(commit_message="خودکار بیک اپ"):
    try:
        g = get_github_client()
        if not g:
            return False
        ...
        try:
            ...
            if e.status == 404:
                repo.create_file(DB_NAME, commit_message, content)
            else:
                st.error(f"GitHub اپ لوڈ میں خرابی: {e}")
                return False
        return True
    except Exception as e:
        st.error(f"GitHub اپ لوڈ میں غیر متوقع مسئلہ: {e}")
        return False
...
def commit_and_backup(conn, message="ڈیٹا محفوظ ہوا"):
    conn.commit()
    if "GITHUB_TOKEN" in st.secrets:
        time.sleep(0.5)
        upload_db_to_github(message)
...
# ==================== 3. اسٹائلنگ ====================
st.set_page_config(page_title="جامعہ ملیہ اسلامیہ | سمارٹ ERP", layout="wide", initial_sidebar_state="collapsed")
st.markdown("""
<style>
    @font-face {
    ...
    * {
    ...
    .best-student-card:hover { transform: scale(1.02); }
    .gold { color: #d4af37; }
    .silver { color: #a0a0a0; }
    .bronze { color: #cd7f32; }
        ...
        with st.container():
            ...
            if st.button("داخل ہوں"):
                ...
                if res:
                    ...
                    st.session_state.selected_page = None  # ڈیش بورڈ دکھانے کے لیے
...
# ==================== 5. مینو اور ڈیش بورڈ سیٹ اپ ====================
if st.session_state.user_type == "admin":
    menu_items = {
        "📊 ڈیش بورڈ": {"icon": "📊", "color": "#1e5631", "page": "admin_dashboard"},
        "📋 یومیہ رپورٹ": {"icon": "📋", "color": "#2e7d32", "page": "daily_report"},
        "🎓 امتحانی نظام": {"icon": "🎓", "color": "#f9a825", "page": "exam_system"},
        "📜 ماہانہ رزلٹ": {"icon": "📜", "color": "#d32f2f", "page": "monthly_result"},
        "📘 پارہ رپورٹ": {"icon": "📘", "color": "#1976d2", "page": "para_report"},
        "🕒 اساتذہ حاضری": {"icon": "🕒", "color": "#f57c00", "page": "teacher_attendance"},
        "🏛️ رخصت منظوری": {"icon": "🏛️", "color": "#7b1fa2", "page": "leave_approval"},
        "👥 یوزر مینجمنٹ": {"icon": "👥", "color": "#00838f", "page": "user_management"},
        "📚 ٹائم ٹیبل": {"icon": "📚", "color": "#ad1457", "page": "timetable_management"},
        "🔑 پاسورڈ تبدیل": {"icon": "🔑", "color": "#6a1b9a", "page": "change_password"},
        "📋 عملہ نگرانی": {"icon": "📝", "color": "#bf360c", "page": "staff_monitoring"},
        "📢 نوٹیفیکیشنز": {"icon": "📢", "color": "#0277bd", "page": "notifications"},
        "📈 تجزیہ": {"icon": "📈", "color": "#2e7d32", "page": "analytics"},
        "🏆 بہترین طلباء": {"icon": "🏆", "color": "#ff8f00", "page": "best_students"},
        "⚙️ بیک اپ": {"icon": "⚙️", "color": "#455a64", "page": "backup_settings"},
    }
else:
    menu_items = {
        "📝 سبق اندراج": {"icon": "📝", "color": "#1e5631", "page": "daily_entry"},
        "🎓 امتحانی درخواست": {"icon": "🎓", "color": "#f9a825", "page": "exam_request"},
        "📩 رخصت درخواست": {"icon": "📩", "color": "#7b1fa2", "page": "leave_request"},
        "🕒 میری حاضری": {"icon": "🕒", "color": "#f57c00", "page": "my_attendance"},
        "📚 میرا ٹائم ٹیبل": {"icon": "📚", "color": "#ad1457", "page": "my_timetable"},
        "🔑 پاسورڈ تبدیل": {"icon": "🔑", "color": "#6a1b9a", "page": "change_password"},
        "📢 نوٹیفیکیشنز": {"icon": "📢", "color": "#0277bd", "page": "notifications"},
    }

# سیشن میں موجودہ صفحہ سیٹ کریں
if "selected_page" not in st.session_state:
    st.session_state.selected_page = "admin_dashboard" if st.session_state.user_type == "admin" else "daily_entry"

# سائڈبار کو چھوٹا اور خوبصورت رکھیں
with st.sidebar:
    st.markdown("## 🕌 جامعہ ملیہ")
    if st.button("🏠 ہوم", use_container_width=True):
        st.session_state.selected_page = "admin_dashboard" if st.session_state.user_type == "admin" else "daily_entry"
        st.rerun()
    st.divider()
    if st.button("🚪 لاگ آؤٹ", use_container_width=True):
        if "GITHUB_TOKEN" in st.secrets:
            upload_db_to_github("لاگ آؤٹ پر بیک اپ")
        st.session_state.logged_in = False
        st.rerun()

# ==================== 6. ڈیش بورڈ (آئیکن گرڈ) دکھانے کا فنکشن ====================
def show_dashboard():
...
# ---------- ایڈمن یومیہ رپورٹ ----------
elif st.session_state.selected_page == "daily_report" and st.session_state.user_type == "admin":
...
# ---------- ایڈمن امتحانی نظام ----------
elif st.session_state.selected_page == "exam_system" and st.session_state.user_type == "admin":
...
# ---------- ایڈمن ماہانہ رزلٹ ----------
elif st.session_state.selected_page == "monthly_result" and st.session_state.user_type == "admin":
...
# ---------- ایڈمن پارہ رپورٹ ----------
elif st.session_state.selected_page == "para_report" and st.session_state.user_type == "admin":
...
# ---------- ایڈمن اساتذہ حاضری ----------
elif st.session_state.selected_page == "teacher_attendance" and st.session_state.user_type == "admin":
...
# ---------- ایڈمن رخصت منظوری ----------
elif st.session_state.selected_page == "leave_approval" and st.session_state.user_type == "admin":
...
# ---------- ایڈمن یوزر مینجمنٹ ----------
elif st.session_state.selected_page == "user_management" and st.session_state.user_type == "admin":
...
# ---------- ایڈمن ٹائم ٹیبل مینجمنٹ ----------
elif st.session_state.selected_page == "timetable_management" and st.session_state.user_type == "admin":
...
# ---------- پاسورڈ تبدیل (دونوں کے لیے) ----------
elif st.session_state.selected_page == "change_password":
...
# ---------- ایڈمن عملہ نگرانی ----------
elif st.session_state.selected_page == "staff_monitoring" and st.session_state.user_type == "admin":
...
# ---------- نوٹیفیکیشنز (دونوں کے لیے) ----------
elif st.session_state.selected_page == "notifications":
...
# ---------- ایڈمن تجزیہ ----------
elif st.session_state.selected_page == "analytics" and st.session_state.user_type == "admin":
    st.header("تجزیہ")
    conn = get_db_connection()
...
# ---------- ایڈمن بہترین طلباء ----------
elif st.session_state.selected_page == "best_students" and st.session_state.user_type == "admin":
...
# ---------- ایڈمن بیک اپ ----------
elif st.session_state.selected_page == "backup_settings" and st.session_state.user_type == "admin":
    ...
    st.markdown("---")
    st.subheader("☁️ GitHub پر دستی بیک اپ")
    if st.button("GitHub پر ابھی اپ لوڈ کریں"):
        if upload_db_to_github():
            st.success("ڈیٹا بیس GitHub پر محفوظ ہو گیا")
        else:
            st.error("بیک اپ ناکام")
    st.markdown("---")
    ...
    table_options = {"اساتذہ (teachers)": "teachers", "طلبہ (students)": "students", "حفظ ریکارڈ (hifz_records)": "hifz_records", "قاعدہ ریکارڈ (qaida_records)": "qaida_records", "عمومی تعلیم (general_education)": "general_education", "امتحانات (exams)": "exams", "پاس شدہ پارے (passed_paras)": "passed_paras", "ٹائم ٹیبل (timetable)": "timetable", "رخصت درخواستیں (leave_requests)": "leave_requests", "نوٹیفیکیشنز (notifications)": "notifications", "عملہ نگرانی (staff_monitoring)": "staff_monitoring"}
...
# ---------- ٹیچر: روزانہ سبق اندراج ----------
elif st.session_state.selected_page == "daily_entry" and st.session_state.user_type == "teacher":
        ...
        if not students:
            ...
            for sid, s, f in students:
                ...
                if sabaq_nagha or sabaq_yad_nahi:
                    ...
                    surah = st.selectbox("سورت", [f"سورۃ {i}" for i in range(1,115)], key=f"surah_{key}")
                ...
                if sq_nagha or sq_yad_nahi:
                    ...
                    for i in range(st.session_state[f"sq_rows_{key}"]):
                        ...
                        p = cols[0].selectbox("پارہ", [f"پارہ {x}" for x in range(1,31)], key=f"sqp_{key}_{i}")
                ...
                if m_nagha or m_yad_nahi:
                    ...
                    for j in range(st.session_state[f"m_rows_{key}"]):
                        cols = st.columns([2,2,1,1])
                        p = cols[0].selectbox("پارہ", [f"پارہ {x}" for x in range(1,31)], key=f"mp_{key}_{j}")
...
# ---------- ٹیچر: امتحانی درخواست ----------
elif st.session_state.selected_page == "exam_request" and st.session_state.user_type == "teacher":
...
# ---------- ٹیچر: رخصت درخواست ----------
elif st.session_state.selected_page == "leave_request" and st.session_state.user_type == "teacher":
...
# ---------- ٹیچر: میری حاضری ----------
elif st.session_state.selected_page == "my_attendance" and st.session_state.user_type == "teacher":
...
# ---------- ٹیچر: میرا ٹائم ٹیبل ----------
elif st.session_state.selected_page == "my_timetable" and st.session_state.user_type == "teacher":
